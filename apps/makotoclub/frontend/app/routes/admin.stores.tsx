...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  return Response.json<LoaderData>({ stores });
}
...
export default function AdminStores() {
  ...
  return (
    <main className="container mx-auto px-4 py-12 space-y-8">
      ...
      <Form method="post" className="grid gap-4 md:grid-cols-2 bg-white/90 border border-slate-200 rounded-2xl p-6 shadow-sm">
        ...
        <Field label="女子給(60分)" name="unitPrice" placeholder="例: 10000" />
          ...
          {stores.map((store) => (
            ...
            >
              ...
              <div className="text-sm text-slate-600">
                総評: {store.averageRating?.toFixed?.(1) ?? "-"} / 女子給:{" "}
