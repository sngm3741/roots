...
export function HelpIcon({ label, containerClassName, iconClassName }: Props) {
  ...
  return (
    <div ref={rootRef} className={`inline-flex ${containerClassName ?? ""}`}>
      <div className="relative inline-flex">
        <button
          type="button"
          aria-label="ヘルプ"
          aria-controls={hasLabel ? id : undefined}
          aria-expanded={hasLabel ? open : undefined}
          disabled={!hasLabel}
          onClick={() => {
            if (!hasLabel) return;
            setOpen((prev) => !prev);
          }}
          className="inline-flex h-3 w-3 items-center justify-center disabled:cursor-default"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
            aria-hidden="true"
            className={iconClassName ?? "h-4 w-4 text-slate-500"}
            fill="currentColor"
          >
        ...
        {hasLabel && open ? (
          <div
            id={id}
            role="tooltip"
            className="absolute left-1/2 top-full z-50 mt-2 w-40 -translate-x-1/2 rounded-xl border border-pink-100 bg-white/95 p-3 text-xs text-slate-700 shadow-lg whitespace-pre-wrap"
          >
            {label}
          </div>
        ) : null}
