...
- ストアカード `StoreCard`
  ...
  - 中段: ステータス `StatRow` (満足度/稼ぎ/待機)、星 `RatingStars`
  - 下段: 件数タグ
- アンケートカード `SurveyCard`
  - 上段: 店舗位置/店名、タグ（訪問時期・年齢・スペック）
  - 中段: `StatRow` (満足度/稼ぎ/待機)
  - 下段: コメント抜粋 + 投稿日
...
- 並び替えバー: 新着/稼ぎ/満足度の 3 ボタン（状態付き）。検索条件は hidden で引き継ぎ。
...
- バリデーション: 年齢/スペック/満足度/数値は範囲チェック。必須未入力でエラーアラート表示。送信中 disable。
- 成功時 `/surveys` リダイレクト。
...
- カードは情報を詰め込み過ぎない。最小必須フィールド（位置/業種/満足度/稼ぎ/待機 + コメント1行）。
